...
export default function BoardCreator({ onBoardCreated }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await api.post("/kanban/boards/", {title, description})
    ...
    return <div className="flex ml-auto">
        <Dialog.Root open={isOpen} onOpenChange={setIsOpen}>
            <Dialog.Trigger asChild>
                <button className="bg-gray-600 text-white px-6 py-3 rounded-lg cursor-pointer hover:bg-gray-700 transition duration-200">
                    <Plus size={16} />
                </button>
            </Dialog.Trigger>
